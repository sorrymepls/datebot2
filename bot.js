...
bot.action("startBot", (ctx) => {
  return ctx.reply(
    `Тоді розпочнемо!`,
    Extra.HTML().markup((m) =>
      m.inlineKeyboard([m.callbackButton("Ну давай уже 😂", "warn")])
    )
  );
});
...
bot.action("restartBot", (ctx) => {
  return ctx.reply(
    ...
    Extra.HTML().markup((m) =>
      m.inlineKeyboard([m.callbackButton("Ох і скатіна!", "startBot")])
    )
  );
});
...
bot.action("warn", (ctx) => {
  return ctx.reply(
    ...
    Extra.HTML().markup((m) =>
      m.inlineKeyboard([m.callbackButton("Готово!", "guide")])
    )
  );
});
...
P.S. Трішки терпіння не завадить, не спіши натискати кнопку декілька раз, вона може трішки думати))
  `,
    Extra.HTML().markup((m) =>
      m.inlineKeyboard([m.callbackButton("Поняу Приняу!", "firstMes")])
    )
  );
});
...
bot.action("tenthMes", (ctx) => {
  ...
  bot.telegram.sendMessage(
    saniaId,
    "Саня, всьо туц, Настя сіла в таксу, з мене півас!!!❤️❤️❤️"
  );
  ctx.replyWithPhoto(
    {
      url: "https://i.imgur.com/likzDBr.jpg",
